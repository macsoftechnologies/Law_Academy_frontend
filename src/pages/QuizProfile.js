...
function QuizProfile() {
  ...
  const quizId = prelimes_test_id || localStorage.getItem("prelimes_test_id");
  ...
  useEffect(() => {
    ...
    const fetchQuiz = async () => {
      try {
        const res = await getQuizzes(1, 1000);
        const all = Array.isArray(res?.data) ? res.data : Array.isArray(res) ? res : [];
        const found = all.find((q) => String(q.prelimes_test_id) === String(quizId));
        setQuiz(found || null);
      } catch (err) {
  ...
  useEffect(() => {
    if (!quiz?.prelimes_id) return;
    const fetchPrelims = async () => {
      try {
        const res = await getPrelims(1, 1000);
        const list = res?.data || [];
        const found = list.find((p) => (p.prelimes_id || p._id) === quiz.prelimes_id);
        setPrelimsName(found?.title || quiz.prelimes_id);
      } catch (err) {
  ...
  const fetchQuestions = useCallback(
    async (page = 1, limit = pageLimit) => {
      ...
      setIsLoading(true);
      try {
        const res = await getQuestion(page, limit, quizId);
  ...
  if (!quiz) {
    return (
      <div className="p-3 text-center">
        ...
        <p className="mt-2">Loading quiz profile...</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mt-3">
      ...
      <div className="card p-3 shadow-sm mb-4">
        ...
        <div className="row mt-3 align-items-start">
          <div className="col-md-4 mb-3">
            <p><b>Title:</b>       {quiz.title       || "N/A"}</p>
            <p><b>Test Number:</b> {quiz.test_number || "N/A"}</p>
            <p><b>Test Type:</b>   {quiz.test_type   || "N/A"}</p>
          </div>
          <div className="col-md-4 mb-3">
            <p><b>No. of Questions:</b> {quiz.no_of_qos || "N/A"}</p>
            <p><b>Duration (mins):</b>  {quiz.duration  || "N/A"}</p>
          ...
          <div className="col-md-4 mb-3">
            ...
            <p>
              <b>Updated At:</b>{" "}
              {quiz.updatedAt ? new Date(quiz.updatedAt).toLocaleString() : "N/A"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="card p-3 shadow-sm mb-4">
        ...
        {/* Showing count + records per page */}
